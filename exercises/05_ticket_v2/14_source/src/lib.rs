...
mod status;
use status::{Status, ParseStatusError};
...
impl Ticket {
    pub fn new(title: String, description: String, status: String) -> Result<Self, TicketNewError> {
        ...
        let x: Result<Status, ParseStatusError> =  Status::try_from(status);
        match x {
            Ok(success) => return         Ok(Ticket {
            title,
            description,
            status: success,
        }),
            Err(msg) => return Err(TicketNewError::StatusInvalid{
                source: msg
            }),
        }
