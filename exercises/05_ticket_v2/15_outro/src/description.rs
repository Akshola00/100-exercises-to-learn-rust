...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if value.is_empty() {return Err("The description cannot be empty".to_string())};
        if value.as_bytes().len() > 500 {return Err("The description cannot be longer than 500 bytes".to_string())};
